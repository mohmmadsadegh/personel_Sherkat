...
class company():
    ...
    def Show_Moshtari(self):
        for item in self.Moshtariyan:        
            print(f"list moshtariyan: **{item.name}**  **{item.famili}**  kalaye sefarshi shoma **{item.kala}**  hast va tedad kala **{item.number_kala}**  adad mibashad  ba ghimat **{item.price}$** **{item.bedehi}**")         
    ...
    def Show_taminkonande(self):
        for item in self.Taminkonandegan:
            print(f"list taminkonandegan **{item.name}**  tahiye shode az **{item.mavad_Avaliye}**  ba ghimate **{item.price_mavad_avaliye}$**  ba tahvil **{item.tahvil}** {item.sabeghKifary}")     
    def Add_personel(self,personel):
        self.personels.append(personel) 
    def Show_personel(self):
        for item in self.personels:
            print(f"list karkonan **{item.name}**  **{item.famili}**   **{item.takhasos}**  **{item.saatKar}** **{item.sabeghKifary}**")                             
class personel(person):
    ...
    boss_All='' 
    sabeghKifary=False 
    def __init__(self, name, famili,takhasos,saatKar,dastmozd,sabeghKifary:str|int|bool) -> None:
        self.name=name
        self.famili=famili
    ...
    def saatKar(self):
        print("GET")    
        return f"{self._saatKar} saat kar mikonad dar rooz"
    @saatKar.setter
...
class Hesabdari(person):
    ...
    def Show_Moshtari(self):
        for item in self.Moshtariyan:        
            print(f" list moshtariyan: **{item.name}**  **{item.famili}**  kalay sefareshi shoma **{item.kala}**  mibashad va tedad kala  **{item.number_kala}** addad mibashad ba ghimat **{item.price}$** **{item.bedehi}**")    
    def Add_taminkonande(self,taminkonande):
        self.Taminkonandegan.append(taminkonande) 
    def Show_taminkonande(self):
        for item in self.Taminkonandegan:
    ...
    def Show_personel(self):
        for item in self.personels:
            print(f" list personel:  **{item.name}**  **{item.famili}**  **{item.takhasos}**  **{item.saatKar}**  **{item.sabeghKifary}**")                             
...
class Boss_company(person):
    ...
    def Show_Moshtari(self):
        for item in self.Moshtariyan:        
            print(f" list moshtariyan: **{item.name} **  **{item.famili}**  kalay sefareshi shoma  **{item.kala}**  mibashad ba tedad sefaresh  **{item.number_kala}**  addad ba ghimat **{item.price}$** **{item.bedehi}**")    
    def Add_taminkonande(self,taminkonande):
        self.Taminkonandegan.append(taminkonande) 
    def Show_taminkonande(self):
        for item in self.Taminkonandegan:
